...
class RadarScanView : RelativeLayout{
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        if( mWidth > mHeight){
            for (i in 0..9) {
                lines.add(Line((mWidth / 2).toFloat(), (mHeight / 2).toFloat(), (mHeight * 0.9 /2).toFloat(), angle))
                angle -= 1.5
            }
            dSize = (mHeight * 0.3 / 2).toInt()
            dlayoutParams = LayoutParams(dSize, dSize)
            dlayoutParams.addRule(RelativeLayout.CENTER_IN_PARENT, RelativeLayout.TRUE)
            radius = (mHeight * 0.9 / 2).toFloat()
